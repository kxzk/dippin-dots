...
If plain `kubectl exec` shows `kms://...` but `aws-env exec` shows a decrypted-looking value, the config is present and the plain probe is invalid.

First principles: `aws-env exec` decrypts env for the process tree it starts. `kubectl exec` starts a separate process with the original pod env. Plain exec is testing the wrong runtime.
